...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # 左边特征提取层
        self.down_block1 = block_model(self.input_channels, down_in[0], down_out[0], self.individual)
        self.down_block2 = block_model(self.input_channels, down_in[1], down_out[1], self.individual)
        self.down_block3 = block_model(self.input_channels, down_in[2], down_out[2], self.individual)
        self.down_block4 = block_model(self.input_channels, down_in[3], down_out[3], self.individual)


        # 右边特征融合层
        self.Up4 = nn.Upsample(scale_factor=(2), mode='nearest')
        self.up_block3 = block_model(self.input_channels, down_out[2] + down_out[3], down_out[2], self.individual)

        self.Up3 = nn.Upsample(scale_factor=(2), mode='nearest')
        self.up_block2 = block_model(self.input_channels, down_out[1] + down_out[2], down_out[1], self.individual)

        self.Up2 = nn.Upsample(scale_factor=(2), mode='nearest')
        self.up_block1 = block_model(self.input_channels, down_out[0] + down_out[1], down_out[0], self.individual)
    ...
    def forward(self, x):
        ...
        x2 = self.Maxpool1(x1)#48
        e2 = self.down_block2(x2)

        x3 = self.Maxpool2(x2)#24
        e3 = self.down_block3(x3)

        x4 = self.Maxpool3(x3)#12
        e4 = self.down_block4(x4)


        #d4 = self.Up4(e4)#24
        d4 = torch.cat((e3, e4), dim=2)  # 将e3特征图与d4特征图横向拼接
        #d4 = d4 + e3
        d4 = self.up_block3(d4)#24

        #d3 = self.Up3(d4)#48
        d3 = torch.cat((e2, d4), dim=2)  # 将e2特征图与d3特征图横向拼接
        #d3=d3 + e2
        d3 = self.up_block2(d3)#48

        #d2 = self.Up2(d3)#96
        d2 = torch.cat((e1, d3), dim=2)  # 将e1特征图与d1特征图横向拼接
        #d2 = d2 + e1
